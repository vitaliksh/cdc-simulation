...
const BALL_COEFF = 1.0;

const HIDE_CONV_B = true; // keep Conveyor B code, but hide it visually

// Read shared CSS palette
const _cs = getComputedStyle(document.documentElement);
const CLR_ACCENT  = _cs.getPropertyValue('--demo-accent').trim()  || '#007acc';
const CLR_BTN     = _cs.getPropertyValue('--demo-btn').trim()     || '#4a5568';
const CLR_BORDER  = _cs.getPropertyValue('--demo-border').trim()  || '#333';
...
// Guide line tuning (single source of truth)

const GUIDE_START_DY = 16;

const GUIDE_END_DX = -20;

const GUIDE_END_DY = -2;
...
// Preload sound effects so first play has no delay
...
document.getElementById('btnSound').onclick = function() {
  ...
  this.innerHTML = state.soundEnabled ? 'ðŸ”Š' : 'ðŸ”‡';
  ...
  draw(ctx) {
    ...
    for (let k = minK; k <= maxK; k++) {
      ...
      if (d >= 0 && d < peri) {
        ...
        const isTopHalf = (d >= 0 && d <= L + Math.PI * R / 2) || (d >= 2 * L + 1.5 * Math.PI * R && d < peri);
...
// Returns the guide line endpoints (used both for drawing and for rolling balls)

function getGuideLine() {

  const start = getPosOnConveyor(convA, convA.geom.L + Math.PI * convA.geom.R / 2, getBallRadiusA(), 0);

  const end = getTableauBGatePoint();

  return {

    start: { x: start.x, y: start.y + GUIDE_START_DY },

    end:   { x: end.x + GUIDE_END_DX, y: end.y + GUIDE_END_DY }
...
function autoStopIfDone() {
  ...
  state.timeouts.push(setTimeout(() => {
    ...
    cloneTitle.style.left = (115 - cloneLeft) + 'px';
  ...
  state.timeouts.push(setTimeout(() => {
    ...
    titleA.style.left = (430 - targetLeftA) + 'px';
...
function updateBalls() {
  ...
  balls.forEach(b => {
    ...
    if (b.state === 'A') {
      ...
      if (d > distFall) {
        ...
        const g = getGuideLine();

        const entry = getTableauBEntryPoint();

        b.state = 'FALL_TO_TAB_B';

        b.fallStartOffsetA = convA.absOffset;

        b.dropStartX = g.start.x;

        b.dropStartY = g.start.y;

        b.dropGateX = g.end.x;

        b.dropGateY = g.end.y;

        b.dropTargetX = entry.x;

        b.dropTargetY = entry.y;
  ...
  balls.forEach(b => {
    ...
    if (b.isSplit && b.timeHitB && (Date.now() - b.timeHitB >= 1000)) {
    ...
    if (b.state === 'EMIT_A') {
    ...
    } else if (b.state === 'FALL_TO_TAB_B') {

      // ~2 seconds at animSpeed=1.0 (about 120 frames @60fps).

      const D_FALL_DIRECT = 120;

      const progress = (convA.absOffset - b.fallStartOffsetA) / D_FALL_DIRECT;

      const t = Math.max(0, Math.min(1, progress));

      b.x = b.dropStartX + (b.dropGateX - b.dropStartX) * t;

      b.y = b.dropStartY + (b.dropGateY - b.dropStartY) * t;

 

      // shift the ball center up from the guide line so the ball touches the guide by its edge

      const gdx = (b.dropGateX - b.dropStartX);

      const gdy = (b.dropGateY - b.dropStartY);
      ...
      // choose the normal that points upward on the screen (negative Y)
      ...
      if (t >= 1.0) {
        ...
        b.state = 'INSERT_TO_TAB_B';

        b.insertStartOffsetA = convA.absOffset;

        b.insertFromX = b.dropGateX;

        b.insertFromY = b.dropGateY;

        b.insertToX = b.dropTargetX;

        b.insertToY = b.dropTargetY;
    ...
    } else if (b.state === 'INSERT_TO_TAB_B') {

      const D_INSERT = 18;

      const progress = (convA.absOffset - b.insertStartOffsetA) / D_INSERT;

      const t = Math.max(0, Math.min(1, progress));

      b.x = b.insertFromX + (b.insertToX - b.insertFromX) * t;

      b.y = b.insertFromY + (b.insertToY - b.insertFromY) * t;

      if (t >= 1.0) {

        if (state.soundEnabled) playFx('tableou_in.mp3');

        pushBitIntoTableauB(b.bit, 0);
...
function drawAll() {
  ...
  if (!HIDE_CONV_B) convB.draw(ctx);

 

  positionTableauBWrapper();

  // draw guide line between Conveyor A and Tableau B (for the falling balls)
  ...
  balls.forEach(b => {
    ...
    draw3DBall(ctx, b.x, b.y, b.r, b.bit, b.isSplit, b.splitSide);
...
function restoreFromIdx(idx) {
  ...
  balls.forEach(b => { if (b.isSplit && b.timeHitB) b.timeHitB = Date.now(); });
